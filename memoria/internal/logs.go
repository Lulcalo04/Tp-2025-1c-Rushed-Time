...
func LogDestruccionDeProceso(pid int, metricas MetricasPorProceso) {
	message := fmt.Sprintf("## PID: %d - Proceso Destruido - Métricas - Acc.T.Pag: %d; Inst.Sol.: %d; SWAP: %d; Mem.Prin.: %d; Lec.Mem.: %d; Esc.Mem.: %d",
...
func LogObtenerInstruccion(pid int, pc int, instruccion string, args ...string) {
	message := fmt.Sprintf("## PID: %d - Obtener instrucción: %d - Instrucción: %s %s",
...
func LogOperacionEnEspacioUsuario(pid int, operacion string, direccionFisica int, size int) {
	message := fmt.Sprintf("## PID: %d - %s - Dir. Física: %d - Tamaño: %d", pid, operacion, direccionFisica, size)
	Logger.Info(message)
	fmt.Println(message)
}

func LogMemoryDump(pid int) {
	message := fmt.Sprintf("## PID: %d - Memory Dump solicitado", pid)
	Logger.Info(message)
	fmt.Println(message)
}
